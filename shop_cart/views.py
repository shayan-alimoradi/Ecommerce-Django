        ...
        if form.is_valid():
            ...
            if check == "yes":
                if product.status is not None:
                    shop = Cart.objects.get(
                        user_id=request.user.id, product_id=id, variant_id=var_id
                    )
                else:
...
def add_to_compare_list(request, id):
    ...
    if request.user.is_authenticated:
        ...
        if qs.exists():
            messages.error(
                request, "This item has already exists in your compare list", "warning"
            )
        else:
            Compare.objects.create(user_id=request.user.id, product_id=id)
    else:
        item = get_object_or_404(Product, id=id)
        qs = Compare.objects.filter(
            user_id=None, product_id=id, session_key=request.session.session_key
        )
        if qs.exists():
            messages.error(
                request, "This item has already exists in your compare list", "warning"
...
def compare_list(request):
    if request.user.is_authenticated:
        ...
        data = Compare.objects.filter(
            session_key=request.session.session_key, user_id=None
        )
        return render(request, "cart/compare_list.html", {"data": data})
